"""Tests for range_suggestion pure helpers (no I/O)."""
...
def test_suggest_n_grids_respects_min_notional():
    # investment*leverage = 1000, min_notional = 100 → ≤ 10 grids
    n = suggest_n_grids(
...
def test_suggest_n_grids_respects_fee_breakeven():
    # Narrow range, fee-dominated: step must be > 2*fee of avg_price.
    # range=2, avg=199, fee=0.001 → step/avg > 0.002 → step > 0.398 → n < 5
    n = suggest_n_grids(
        price_low=Decimal(198),
        price_high=Decimal(200),
        investment_usdt=Decimal(10_000),
        leverage=10,
...
def test_suggest_n_grids_min_is_2():
    # Invalid inputs collapse to the floor
    assert suggest_n_grids(
        price_low=Decimal(100),
