"""Markov chain market regime detection for grid strategy adaptation.
...
one of four regimes using a discrete first-order Markov chain:
  ...
  RANGING      — low trend, moderate volatility  → grid ideal
  TRENDING_UP  — strong uptrend                 → grid long OK, neutral/short risky
  TRENDING_DOWN — strong downtrend              → grid short OK, neutral/long risky
  VOLATILE     — high volatility, unclear dir   → grid risky regardless of direction
...
# ---------------------------------------------------------------------------
# Grid action helper — pure, depends on regime + bot direction
# ---------------------------------------------------------------------------

def grid_action_for_regime(
    regime: MarketRegime,
    direction: str,
) -> str:
    """Return the recommended grid action for a given regime and direction.

    Returns one of "run", "pause".

    Logic:
      RANGING     → always "run"  (grid ideal)
      VOLATILE    → always "pause"
      TRENDING_UP → "run" for long, "pause" for neutral/short
      TRENDING_DOWN → "run" for short, "pause" for neutral/long
    """
    ...
    if regime == MarketRegime.TRENDING_UP:
        return "run" if direction == "long" else "pause"
    if regime == MarketRegime.TRENDING_DOWN:
        return "run" if direction == "short" else "pause"
    return "run"  # unreachable — defensive
